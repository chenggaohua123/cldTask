...
public class SendCreateShipmentToCardHolder implements SendEmailService {
	...
	public int sendEmail(EmailSendInfo info,EmailInfo emailInfo) {
		...
		entity.setTemplateName("uploadtraceno_hooppay_holder.ftl");
		entity.setEmailSendTo(result.getCardHoldEmail());
		List<MerchantEmailConfigInfo> emailConfigs = transactionManageDao.queryMerEmailConfigByMerNoAndTerNo(result.getMerNo(), result.getTerNo(),"4");
		for(MerchantEmailConfigInfo emailConfig:emailConfigs ){
			...
			context.put("Iogistics", result.getIogistics());
			context.put("trackno", result.getTrackNo());
			context.put("IogisticsUrl", result.getIogisticsUrl());
			context.put("email", result.getCardHoldEmail());
			context.put("merwebsite", result.getCardHoldEmail());
