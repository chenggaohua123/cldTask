...
public class SendAddRefuseToMerchant implements SendEmailService {
	...
	public int sendEmail(EmailSendInfo info,EmailInfo emailInfo) {
		...
		Complaint complain = transactionManageDao.queryComplaintTransByTradeNoAndComplainType(result.getTradeNo(), "1");
		if(null == complain){
			return 0;
		}
		if(complain.getMerchantType()==2){//海外商户
			entity.setTemplateName("protest_fhtpay_merchant_en.ftl");
		}else{
			entity.setTemplateName("protest_hooppay_merchant.ftl");
		}
		...
		List<MerchantEmailConfigInfo> emailConfigs = transactionManageDao.queryMerEmailConfigByMerNoAndTerNo(result.getMerNo(), result.getTerNo(),"6");
